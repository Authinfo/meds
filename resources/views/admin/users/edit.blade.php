    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="text" name="number" id="number" value="{{ old('number', $user->number) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('idtype') ? 'is-invalid' : '' }}" name="idtype" id="idtype" required>
                    ...
                    @foreach(App\Models\User::IDTYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('idtype', $user->idtype) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('noid') ? 'is-invalid' : '' }}" type="text" name="noid" id="noid" value="{{ old('noid', $user->noid) }}">
                @if($errors->has('noid'))
                    <div class="invalid-feedback">
                        {{ $errors->first('noid') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('jk') ? 'is-invalid' : '' }}" name="jk" id="jk" required>
                    ...
                    @foreach(App\Models\User::JK_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('jk', $user->jk) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('jk'))
                    <div class="invalid-feedback">
